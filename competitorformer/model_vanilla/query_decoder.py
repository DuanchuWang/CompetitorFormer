...
from .transformer import TransformerDecoder, TransformerDecoderLayer, TransformerDecoderCrossLayer
...
class QueryDecoder(nn.Module):
    ...
    def __init__(
        ...
        decoder_layer = TransformerDecoderLayer(d_model, nhead, quant_grid_length, grid_size, rel_query, rel_key, rel_value, hidden_dim,
                                        dropout, activation_fn, normalize_before=False, RCA = RCA, RRE = RRE)
        ...
        self.decoder = TransformerDecoder(num_query, num_class, decoder_layer, decoder_layer_cross, num_layer, decoder_norm,
        ...
        #trunc_normal_(self.refpoint_embed.weight, std=.02)
    ...
    def prediction_head(self, query, mask_feats, batch_offsets, input_ranges, ref_points):
        ...
        return pred_bboxes, pred_masks, attn_masks
    ...
    def forward_iter_pred(self, x, pos, batch_offsets, epoch):
        ...
        intermediate_results, ref_points, mask_feats_batched, self_attn, intermediate_class_logits, intermediate_IOU = self.decoder(tgt=query.new_zeros(num_queries, B, d_model),
        ...
        for i in range(len(intermediate_results)):
            ...
            pred_bboxes, pred_masks, attn_masks = self.prediction_head(ouptut_i, mask_feats_list[i+1], batch_offsets, input_ranges, ref_points[i])
            # prediction_labels.append(pred_labels)
            # prediction_scores.append(pred_scores)
            pred_labels = intermediate_class_logits[i]
            pred_scores = intermediate_IOU[i]
            prediction_labels.append(pred_labels)
            prediction_scores.append(pred_scores)
