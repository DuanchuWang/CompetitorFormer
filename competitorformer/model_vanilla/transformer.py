...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_query, num_class, decoder_layer, decoder_layer_cross, num_layers, norm=None, return_intermediate=False, nhead=8, d_model=256, temperature=10000, pos_type="fourier", attn_mask_thresh=0.5, QCL = False):
        super().__init__()
        ...
        self.QCL = QCL
        if QCL:
            self.Leader_embed = nn.ModuleList([copy.deepcopy(nn.Embedding(num_query, d_model)) for _ in range(num_layers - 1)])
            self.Loser_embed = nn.ModuleList([copy.deepcopy(nn.Embedding(num_query, d_model)) for _ in range(num_layers - 1)])
            self.Update_embed = nn.ModuleList([copy.deepcopy(nn.Linear(2 * d_model, d_model)) for _ in range(num_layers - 1)])
            self.Update_output = nn.ModuleList([copy.deepcopy(nn.Linear(2 * d_model, d_model)) for _ in range(num_layers - 1)])
        # self.LN_output = nn.ModuleList([copy.deepcopy(nn.LayerNorm(d_model)) for _ in range(num_layers - 1)])
        self.out_class = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, num_class + 1))
    ...
    def forward(self, tgt, memory, input_ranges, coords_float, mask_feats_batched, lengths,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.QCL:
                if layer_id > 0:
                    # Leader_sort_embedding/Loser_sort_embedding: [N, B, M]
                    Leader_sort_embedding = Get_expand_SORT_embeding(self.Leader_embed[layer_id - 1].weight, output, Fighters)
                    Loser_sort_embedding = Get_expand_SORT_embeding(self.Loser_embed[layer_id - 1].weight, output, Fighters)
                    mask = (Leaders == 1)
                    # Leaders [B, N]
                    # mask: [N, B, M]
                    mask = mask.transpose(0, 1).unsqueeze(-1).expand_as(Loser_sort_embedding)
                    pos_seq_embed = torch.where(mask, self.Leader_embed[layer_id - 1].weight.unsqueeze(1).expand_as(Loser_sort_embedding), self.Loser_embed[layer_id - 1].weight.unsqueeze(1).expand_as(Loser_sort_embedding))
                    # mask_iv: [N, B, M]
                    mask_iv = (Leaders == -1)
                    mask_iv = mask_iv.transpose(0, 1).unsqueeze(-1).expand_as(Loser_sort_embedding)
                    neg_seq_embed = torch.where(mask_iv, Leader_sort_embedding, Loser_sort_embedding)
                    # neg_seq_embed/pos_seq_embed: [N, B, M]
                    concat_features = torch.concat((pos_seq_embed, neg_seq_embed), dim=-1)
                    # concat_embed: [N, B, M]
                    concat_embed = self.Update_embed[layer_id-1](concat_features) 
                    output = self.Update_output[layer_id-1]( torch.concat((output, concat_embed), dim=-1) )
            # ==================
            ...
            # ==== Start of Fitgher ====
            # [N, B, 19]
            class_logits = self.out_class(output_norm).transpose(0, 1)
            intermediate_class_logits.append(class_logits)
            # [N, B, 1] 
            Scores = self.out_score(output_norm).transpose(0, 1)
            intermediate_IOU.append(Scores)
            if self.QCL:
                Leaders, Fighters, SA_mask = Get_Fighter_in_TS_with_index_SAMask_Original(pred_masks, class_logits, Scores.squeeze(-1), mode = "times")
            # ==== End of Fitgher ====
        ...
        if self.return_intermediate:
            return [intermediate,ref_points,memory_list,self_attn, intermediate_class_logits, intermediate_IOU]
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, quant_grid_length, grid_size, rel_query, rel_key, rel_value, dim_feedforward=2048, dropout=0.1,
                 activation="relu",
                 normalize_before=False, RCA = False, RRE = False):
        super().__init__()
        ...
        self.sa_quant_grid_length = 35
        self.sa_grid_size = 0.02
        # Competitor SA
        if RRE:
            self.Sa_table_k = nn.Parameter(torch.zeros(nhead, d_model//nhead, 2 * self.sa_quant_grid_length))
            trunc_normal_(self.Sa_table_k, std=.02)
            self.Sa_table_q = nn.Parameter(torch.zeros(nhead, d_model//nhead, 2 * self.sa_quant_grid_length))
            trunc_normal_(self.Sa_table_q, std=.02)
            self.Sa_table_v = nn.Parameter(torch.zeros(nhead, d_model//nhead, 2 * self.sa_quant_grid_length))
            trunc_normal_(self.Sa_table_v, std=.02)

        else:
            self.Sa_table_q = None
            self.Sa_table_k = None
            self.Sa_table_v = None
        # RCA 
        self.RCA = RCA
    ...
    def forward_post(self, tgt, memory, query_coords_float, key_coords_float,
        ...
        if SA_Masks is not None:
            SA_idx = torch.div(SA_Masks, self.sa_grid_size, rounding_mode='floor').long()
            SA_idx[SA_idx < -self.sa_quant_grid_length] = -self.sa_quant_grid_length
            SA_idx[SA_idx > self.sa_quant_grid_length - 1] = self.sa_quant_grid_length - 1
            SA_idx += self.sa_quant_grid_length
        else:
            SA_idx = None
        # ==== RRE ====
        ...
        tgt2, attn1 = self.self_attn(q, k, value=v, attn_mask=tgt_mask,
                              ...
                              SA_masks = SA_idx,
                              SA_Table_q = self.Sa_table_q,
                              SA_Table_k = self.Sa_table_k,
                              SA_Table_v = self.Sa_table_v)
    ...
    def forward(self, tgt, memory, query_coords_float, key_coords_float,
        ...
        return self.forward_post(tgt, memory, query_coords_float, key_coords_float, tgt_mask, memory_mask,
                                 tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos, query_sine_embed, is_first, SA_Masks)
...
class TransformerDecoderCrossLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory, query_coords_float, key_coords_float,
        ...
        tgt2 = self.cross_attn(query=q,
                                   ...
                                   relative_pos_query_table=self.relative_pos_query_table if self.rel_query else None, 
                                   relative_pos_key_table=self.relative_pos_key_table if self.rel_key else None, 
                                   relative_pos_value_table=self.relative_pos_value_table if self.rel_value else None, RCA = False)[0]
        # ========== End of Cross-Attention =============
