...
from competitorformer.utils import cuda_cast, rle_encode
...
class CompetitorFormer(nn.Module):
    ...
    def train(self, mode=True):
        super(CompetitorFormer, self).train(mode)
    ...
    def build_label_matrix_vectorized(self, sp_instance_label):

        # use broadcast to avoid explict loop
        label_matrix = (sp_instance_label.unsqueeze(1) == sp_instance_label.unsqueeze(0)).float()
        return label_matrix
    ...
    def loss(self, 
        ...
        for i in range(batch_size):
            ...
            sp_instance_label_m = self.build_label_matrix_vectorized(sp_instance_label)
