...
class ScanNetEval(object):
    ...
    def __init__(self, class_labels, min_npoint=None, iou_type=None, use_label=True, compute_rc=True, logger=None):
    ...
    def evaluate_matches(self, matches):
        ...
        rc = np.zeros((len(dist_threshes), len(self.eval_class_labels), len(ious)), np.float64) if self.compute_rc else None
    ...
    def compute_averages(self, aps, rcs=None):
        d_inf = 0
    ...
    def print_results(self, avgs):
        sep = ''
        col1 = ':'
        has_rc = self.compute_rc and 'all_rc' in avgs
        ...
        line += col_fmt.format('AP') + sep
        line += col_fmt.format('AP_50%') + sep
        line += col_fmt.format('AP_25%') + sep
        if has_rc:
            line += col_fmt.format('AR') + sep
            line += col_fmt.format('RC_50%') + sep
            line += col_fmt.format('RC_25%') + sep
        ...
        for (li, label_name) in enumerate(self.eval_class_labels):
            ...
            line += sep + num_fmt.format(ap_avg) + sep
            line += sep + num_fmt.format(ap_50o) + sep
            line += sep + num_fmt.format(ap_25o) + sep
            if has_rc:
                rc_avg = avgs['classes'][label_name]['rc']
                rc_50o = avgs['classes'][label_name]['rc50%']
                rc_25o = avgs['classes'][label_name]['rc25%']
                line += sep + num_fmt.format(rc_avg) + sep
                line += sep + num_fmt.format(rc_50o) + sep
                line += sep + num_fmt.format(rc_25o) + sep
            self.logger.info(line)
        ...
        line += num_fmt.format(all_ap_avg) + sep
        line += num_fmt.format(all_ap_50o) + sep
        line += num_fmt.format(all_ap_25o) + sep
        if has_rc:
            all_rc_avg = avgs['all_rc']
            all_rc_50o = avgs['all_rc_50%']
            all_rc_25o = avgs['all_rc_25%']
            line += num_fmt.format(all_rc_avg) + sep
            line += num_fmt.format(all_rc_50o) + sep
            line += num_fmt.format(all_rc_25o) + sep
        self.logger.info(line)
        self.logger.info('#' * lineLen)
        self.logger.info('')
